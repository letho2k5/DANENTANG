...
const menuItems = [
  { name: 'Trang chủ', route: 'index', icon: 'home-outline' },
  { name: 'Giỏ hàng', route: 'cart', icon: 'cart-outline' },
  { name: 'Yêu thích', route: 'favourite', icon: 'heart-outline' },
  { name: 'Đơn hàng đang xử lý', route: 'orders', icon: 'time-outline' },     // Tab nhanh (chỉ đơn mới)
  { name: 'Lịch sử đơn hàng', route: 'history', icon: 'receipt-outline' },   // THÊM MỚI – Toàn bộ đơn
  { name: 'Hồ sơ cá nhân', route: 'profile', icon: 'person-outline' },
];
        ...
        <View style={styles.sidebarContainer} onStartShouldSetResponder={() => true}>
          ...
          {menuItems.map((item) => (
            <TouchableOpacity
              ...
              onPress={() => handleNavigate(item.route)}
            >
              <Ionicons name={item.icon as any} size={24} style={styles.menuIcon} />
              <Text style={styles.menuText}>{item.name}</Text>
            </TouchableOpacity>
          ))}
          ...
          <TouchableOpacity style={[styles.menuItem, styles.logoutItem]} onPress={confirmLogout}>
            <Ionicons name="log-out-outline" size={24} style={[styles.menuIcon, { color: '#E53935' }]} />
